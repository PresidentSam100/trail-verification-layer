...
5. The bundle is stored immutably in the private SQLite database.
6. `recover` creates one child bundle. A second recovery request creates a blocked bundle and requires escalation.
7. `verify` runs only built-in checks and commands or probes named by the repository owner in `.trailrc.json`.
